...
# mycursor.execute('''drop table if exists Genre CASCADE CONSTRAINTS PURGE''')
...
#mycursor.execute('''create table AVu(idVu int primary key auto_increment, idFilm int, idUser int, CONSTRAINT fk_idFilm FOREIGN KEY (idFilm) REFERENCES Film(idFilm), CONSTRAINT fk_idUser FOREIGN KEY (idUser) REFERENCES Utilisateur(idUser))''')

#mycursor.execute('''create table AAime(idAime int primary key auto_increment, idFilm int, idUser int, CONSTRAINT fk_idFilm2 FOREIGN KEY (idFilm) REFERENCES Film(idFilm), CONSTRAINT fk_idUser2 FOREIGN KEY (idUser) REFERENCES Utilisateur(idUser))''')
...
# mycursor.execute('''Insert into etudiants values(4,%s)''',(ajout,))
# mydb.commit()
